...
if st.button('데이터 불러오기'):
    if not all([customer, audit_no, auditor, manager, verifier, scope, standard, nc_grade, audit_type, issue_date, corrective_action_date]):
        ...
        with st.spinner('데이터를 불러오고 있습니다...'):
            ...
            try:
                ...
                if data:
                    st.session_state['data_loaded'] = True  # 데이터 로드 상태 저장
                    # 직접 데이터 세션 상태에 할당
                    st.session_state.modified_data = {
                        'nc_content': data[2],
...
# 데이터가 성공적으로 불러와졌을 때 수정 완료 버튼 표시
if st.session_state.get('data_loaded', False):
    # 최종 수정 완료 버튼
    if st.button('수정 완료'):
        st.success("모든 수정이 완료되었습니다.")
        st.session_state['data_modified'] = True  # 수정 완료 상태 저장
...
# 수정 완료 상태 확인 후 부적합 보고서 생성 버튼 표시
